...
! Regolith levels ip1 list
    integer, dimension(:), pointer    :: ip1_reg
...
! Regolith (hardcoded to 14 levels)
         if ( vars(i)%nlevs == 14 ) then
            if ( .not. do_reg ) then
!             Get the multiple handles for the regolith levels
              ier = fstinl(iun, nx, ny, nz, -1, ' ', -1, -1, -1, '',         &
                           vars(i)%varname, list, nlist, vars(i)%nlevs)
              if ( associated(ip1_reg) ) deallocate(ip1_reg)
              allocate (ip1_reg(nlist))
              do k = vars(i)%nlevs, 1, -1 
                ier = fstprm(list(k), dateo, deet, npas, ni, nj, nk, nbits,  &
                          datyp, ip1, ip2, ip3, typvar, nomvar, etiket,      &
                          grtyp, ig1, ig2, ig3, ig4, swa, lng, dltf,         &
                          ubc, extra1, extra2, extra3)
                ip1_reg(k) = ip1
...
! For Regolith fields
          elseif (vars(n)%nlevs == size(ip1_reg)) then
             n_reg = n_reg + 1
...
! For Regolith fields
          if ( do_reg .and. fields(n)%nlevs == size(ip1_reg)) then
            do k = size(ip1_reg), 1, -1
              ier = fstlir(fields(n)%val(:,:,k), iun, nx, ny, nz,         &
                          -1, etiket, ip1_reg(k), -1, -1, typvar, nom)
               write(33) ((fields(n)%val(i,j,k),i=1,ni),j=1,nj )
            enddo
            cycle
          endif
          ...
          if ( fields(n)%stag ) then             ! Staggered levels first
            do jk = nk, 2, -1
              ...
              ier = fstlir(fvalue, iun, nx, ny, nz,         &
                          -1, etiket, ip1m(k), -1, -1, typvar, nom)
            ...
            enddo
            write(32) ((fields(n)%val(i,j,1),i=1,ni),j=1,nj )  ! Dummy write
!                                                  Thermo levels next
          else
            do k = nk, 1, -1
              ier = fstlir(fvalue, iun, nx, ny, nz,         &
          ...
          if ( do_interp ) then
            ...
            do j = 1, nj
              do i = 1, ni
                 pres(:) = ps(i,j) * hybt(:)
                 call spline1(pres, fields(n)%val(i,j,:), nk, bign, bign, sg)
                 ...
                 do k = 1, nk
                    if (pres(nk) > slev(k)) then
                       l = klo(k)
                       m = khi(k)
                       fld_ez(i,j,k) = wk1(k) * fields(n)%val(i,j,l)       &
                                      +  wk3(k) * fields(n)%val(i,j,m)     &
